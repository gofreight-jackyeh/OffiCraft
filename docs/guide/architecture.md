...
這份帶你看懂 OffiCraft 底下**怎麼運作**：什麼東西跑在哪、成員怎麼算「線上」、它怎麼替你做事、更新怎麼送到你機器上。
...
| 元件 | 是什麼 | 職責 |
| --- | --- | --- |
| **ocserverd**（server） | 一支 Go 服務 | **唯一真相來源**：身分、聊天、卡片、任務、學習筆記全在它上面。同時提供三個介面（REST + SSE + MCP），內嵌控制台網頁，資料存本機 SQLite。**只綁 loopback。** |
...
![架構圖：owner 透過控制台指揮，所有連線都連到唯一真相來源 server；每台機器上一個 warden 依 server 指令起停跑在 tmux 裡的成員](assets/architecture-overview.svg)


`GitHub Releases ──self-update──▶ ocserverd`（見最後一節）。所有連線都連到 server 這一個真相來源；成員之間**從不**
...
這是 OffiCraft 一個刻意做得很乾淨的地方：**成員在不在線上，純粹由「它此刻有沒有持著那條 SSE 連線」判定出來。**
...
- 這是 online 的**唯一權威來源**：控制台上那顆 presence 圓點、監控頁、其他成員查名冊看到的線上狀態，讀的都是同一份。

好處是診斷不再「鬼打牆」：成員被判離線時，一眼看得出是誰的問題——**成員自己沒連上（它掛了，交給它機器上的
warden 重拉）**，還是**warden 沒連上（warden 掛了，跟成員在不在線上無關）**。warden 與成員**各自自證線上、互不推斷**。

> 開機還有一個過渡態 **waking（喚醒中）**，而它是 **server 在派出開機命令的那一刻自己記下的**，不是等成員回報才算數
> （成員起手時也會報一次，但那時它已經是 waking 了）。這樣才分得出「**叫過但沒起來**」與「**根本沒人叫過**」——
> 兩者以前在圓點上長得一模一樣。等它掛上 SSE 連線就是 **online**；若 **90 秒**內始終沒連上，這次喚醒算失敗，
> 圓點自己退回 offline。所以圓點你會看到 Waking → Online，喚不起來時則是 Waking → Offline。
...
- **context 逼近上限（server 自動換手）**——最常見的，而且是 **server 驅動**的：成員**看不到自己的 context 用量**、也不會自己盯著它換手；是 **server 偵測**到某個成員的 context 逼近上限時，自動編排這一手退場、原地重生下一手（門檻 owner 可在 **設定 › 參數調整** 調：**Claude 成員是記憶用量的百分比，Codex 成員是做過幾輪 context compaction**，兩組分開調）。這一條有**兩段門檻**：先到**第一段**時只是請它開始收尾，不催；還一路升到**第二段**才會開始計時。
- **owner 主動 refocus**——你在成員面板按 **Refocus**，要它把記憶壓乾淨、換一手更清爽的重開。
- **owner 改機器或換 model／runtime／effort**——你改了要它換一台機器跑、或換個模型跑，那一手也得先收尾才能重開，改動才會生效。
- **成員自請 restart**——不是因為 context 快滿（那條由 server 管），而是成員自己判斷這一手該收了（例如剛結束一大段工作、想帶著整併過的記憶重開），主動請求換手。
- **token 快到期（server 自動換手）**——一個在線 session 的 agent token 進入**最後一小時**時，server 會自動對它開一條普通〈停止〉。這一條**沒有人按、也不會有畫面通知你**：token 得重新簽發，而重新簽發要換一手，所以它跟其他觸發走同一條路。⚠️ 這一段以前漏了它（只寫「四種」），而 [下線流程](../design/offboard-flow.md) 一直是對的——**同一件事兩份說法，這裡是漏的那份。**

**收尾怎麼做。** 換手不是說走就走，退場的那一手要先把交接做乾淨。要它做哪幾件事，寫在 **設定 › 全域情境 › 停止**這份文件裡——**那份是權威版，你可以直接編輯**，而且 server 會在要收掉某個成員的那一刻把它**送到那個成員手上**（不必靠它還記得開機時讀過什麼；那時候開機說明多半已經被壓縮掉了）。

出廠版的形狀大致是：報 stopping（告訴 server 我要退場了，presence 進入收尾狀態）→ 把任務進度與步驟備註寫回 server → 整併 lessons → 給自己留一份交接 baton → 收拾暫存 → 報 stopped。**細節以那份文件的現況為準**，這裡只是讓你知道有這麼一段。

**幾乎每一種都沒有時鐘。** 會開始倒數的**只有兩種**，而且都叫「加速停止」：**context 升到第二段門檻**（它真的快撐不住了，寧可
硬收，也不讓一個半死的 session 卡在那裡占著位子），以及**你自己按下加速停止**。兩者共用同一個秒數（預設 **120 秒**，可在
設定調，見 [設定與參數](settings.md)），逾時沒收完 server 就**下令強制回收**、由 warden 執行。

**其餘全部沒有時鐘**：context 的第一段門檻、你按 Refocus、你改機器或換 model、成員自請 restart、token 快到期——這些都只是把收尾程序送到它
手上，然後**等它自己回報收完**。沒人替它計時。它要是再也不回話，收口就在你手上：按 **Accelerated stop（加速停止）** 給它一個
明確的死線（它會被告知），或直接按 **Force stop（強制停止）** 收掉。**這兩顆都要你動手**——server 不會自己起一條時鐘，
見 [成員與外包](members.md)。

⚠️ **這個預設是反過來的（owner 2026-08-21）。** 以前是「除了 Refocus 以外都上 120 秒」，所以每新增一種觸發都會**不小心**帶著
一條死線。現在反過來：**要上時鐘是特例，而且得逐一指名**。看到一個成員停在收尾狀態很久，先假設它本來就沒有時鐘，不是卡住。

⚠️ **「下線」不是這條路，而且那條完全不上時鐘。** 把成員關掉（desired_state 轉 offline）之後，server **不會**在任何秒數
之後幫你硬收：它一直等，直到成員自己回報收完、或是**你親自按加速停止／強制下線**。所以下線之後看到它還掛在收尾狀態，不是卡住，
是在等你或等它——詳見 **〈停止〉** 那份文件。

**接棒怎麼接。** 舊的一走，**server 原地重生一個新的它**（同一個身分、同一個角色），warden 在它機器上把新
session spawn 起來。新的它開機不是從零開始：先去 **peek** 一眼有沒有交接快照，有就把那份 **resume 快照讀回來**
（上一手留的 baton＋server 上的任務狀態），於是它一起手就知道自己是誰、正在做什麼、下一步要幹嘛。開機途中它
處在 **waking（喚醒中）**（server 派出開機命令當下就記下了），等它掛上那條 SSE 長連線（`listen`）就算
**online**——這時它就正式接棒了。
...
- 成員可以被**派到那台機器**上跑（每台機器有自己的 warden，依 server 指令在本機起停成員）。
...
- **只綁 loopback、資料存本機 SQLite** → 你的資料不會因為裝了它就上網。要從外面連得自己開一條 tunnel 或 VPN（見 [mobile.md](mobile.md)）。
  ⚠️ 這一條是整套安全性的地基：**你自己開的那條對外通道，就是把地基挖掉的那一鏟**。開了之後請一併開啟兩步驟驗證（頭像 →〈兩步驟驗證〉），細節與復原方式見 [mobile.md](mobile.md)。
- **server 是唯一真相來源** → 你在控制台看到的、成員彼此看到的，永遠是同一份；不會有「這台記一套、那台記另一套」的分裂。
- **online 純由連線判定** → 狀態不會過期、不會自相矛盾，診斷一眼看穿是誰掛了。
